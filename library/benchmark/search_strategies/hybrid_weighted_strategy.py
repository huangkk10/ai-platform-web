...
class HybridWeightedStrategy(BaseSearchStrategy):
    ...
    def _weighted_merge(
        ...
        for result in section_results:
            # ✅ 修正：從 metadata.id 提取文檔 ID
            doc_id = result.get('metadata', {}).get('id') or result.get('document_id') or result.get('id')
            if not doc_id:
                self._log(f"  ⚠️ 段落結果缺少 document_id: {result.keys()}", level='warning')
                continue
            
            # ✅ 修正：優先使用 score，其次 similarity
            score = result.get('score', 0) or result.get('similarity', 0)
            weighted_score = score * section_weight
            
            merged_dict[doc_id] = {
                **result,
                'document_id': doc_id,  # ✅ 明確設定 document_id
                'section_score': score,
        ...
        for result in document_results:
            ...
            doc_id = result.get('metadata', {}).get('id') or result.get('document_id') or result.get('id')
            if not doc_id:
                self._log(f"  ⚠️ 全文結果缺少 document_id: {result.keys()}", level='warning')
                continue
            
            # ✅ 修正：優先使用 score，其次 similarity
            score = result.get('score', 0) or result.get('similarity', 0)
            weighted_score = score * document_weight
            
            if doc_id in merged_dict:
                # 已存在（同時在段落和全文中）
                merged_dict[doc_id]['document_score'] = score

...
echo "🎯 API 回應時間測試..."
echo "測試配置 API..."
time curl -s "http://10.10.172.127/api/rvt-guide/config/" > /dev/null && echo "✅ RVT Guide Config API 正常" || echo "❌ RVT Guide Config API 異常"

echo ""
echo "測試優化的分析 API..."
time curl -s "http://10.10.172.127/api/rvt-analytics/overview/?days=7" > /dev/null && echo "✅ Analytics Overview API 正常" || echo "❌ Analytics Overview API 異常"

echo ""
echo "📱 可用的管理介面:"
echo "  • Celery 任務監控: http://10.10.172.127:5555"
echo "  • Portainer 容器管理: http://10.10.172.127:9000"
echo "  • Adminer 資料庫: http://10.10.172.127:9090"
echo ""
echo "🔄 監控完成 - $(date)"
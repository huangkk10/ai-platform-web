...
# 配置
# ============================================================
# 在容器內使用 localhost:8000 直接連接 Django
# 在容器外使用 127.0.0.1 透過 Nginx 連接
import socket
def get_api_url():
    """根據環境自動選擇 API URL"""
    # 嘗試連接 localhost:8000 (容器內)
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(1)
        s.connect(('localhost', 8000))
        s.close()
        return "http://localhost:8000/api/saf/smart-query/"
    except:
        pass
    # 使用外部 URL
    return "http://127.0.0.1/api/saf/smart-query/"

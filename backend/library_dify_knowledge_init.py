...
class DifyKnowledgeSearchHandler:
    ...
    def _fallback_search(self, query, limit=5, threshold=0.7, **kwargs):
        """備用搜索實現（接受所有參數但不使用）"""
        self.logger.warning(f"備用搜索被調用: query='{query}', limit={limit}, threshold={threshold}")
        return []
            ...
            if knowledge_type == 'know_issue':
                ...
                if self.vector_search_available and self.search_rvt_guide_with_vectors:
                    try:
                        ...
                        self.logger.info(f"RVT Guide 向量搜索結果: {len(results)} 條 (mode={search_mode}, stage={stage})")

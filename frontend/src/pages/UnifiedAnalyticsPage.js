...
const UnifiedAnalyticsPage = () => {
  ...
  // Assistant 選擇器 - 從 localStorage 讀取上次選擇，預設為 RVT
  ...
  // 當 Assistant 類型改變時，儲存到 localStorage
  useEffect(() => {
    localStorage.setItem('selectedAnalyticsAssistant', selectedAssistant);
  }, [selectedAssistant]);

  useEffect(() => {
    console.log('🔥 UnifiedAnalyticsPage useEffect 觸發');
    console.log('🔥 認證狀態:', { isAuthenticated, user });
    console.log('🔥 當前 Assistant:', selectedAssistant);
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const [overviewResponse, questionResponse, satisfactionResponse] = await Promise.all([
        fetch(`${overviewEndpoint}?days=${selectedDays}${selectedUserId ? `&user_id=${selectedUserId}` : ''}`, {
          credentials: 'include',
          headers: {
        ...
        fetch(`${questionsEndpoint}?days=${selectedDays <= 30 ? selectedDays : 7}&mode=smart`, {
          credentials: 'include',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
        ...
        fetch(`${satisfactionEndpoint}?days=${selectedDays}&detail=true`, {
          credentials: 'include',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',

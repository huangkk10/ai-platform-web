...
def test_image_ocr(image_path):
    ...
    # 先上傳圖片取得 upload_file_id
    upload_url = config.api_url.replace('/chat-messages', '/files/upload')
    ...
    try:
        ...
        upload_file_id = upload_data.get('id')
        print(f"  ✅ 圖片上傳成功")

...
- ✅ **聊天訊息渲染**（使用 `MessageFormatter`，已添加 HTML 支援） 🆕
  - RVT Assistant 聊天頁面
  - Protocol Assistant 聊天頁面
  - 所有使用 MessageFormatter 的聊天介面

### 完全支援的頁面（原本就有配置）
- Guide 預覽頁面（使用 `ReactMarkdown` + `rehypeRaw`）
...
**2025-11-02 (第一次更新)**：
- ✅ 初始實施完成
- ✅ markdown-it 配置更新（MarkdownEditorLayout）
- ✅ ReactMarkdown 添加 rehypeRaw（DevMarkdownTestPage）
- ✅ 清理未使用的 import
- ✅ 前端編譯成功
- ✅ 用戶確認測試頁面功能正常

**2025-11-02 (第二次更新 - 聊天訊息支援)** 🆕：
- ✅ 為 MessageFormatter 添加 HTML 支援
- ✅ 修改檔案：`frontend/src/hooks/useMessageFormatter.js`
- ✅ 添加 `import rehypeRaw from 'rehype-raw'`
- ✅ 在 markdownConfig 中添加 `rehypePlugins: [rehypeRaw]`
- ✅ 前端容器重啟成功
- ✅ 編譯成功（webpack compiled successfully）
- ⏳ 等待用戶測試聊天頁面 `<br>` 標籤效果
